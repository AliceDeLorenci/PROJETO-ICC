...
void loop(){
  ...
  char nome[] = "Antialergico";
  char qtdd[] = "20 ml";
  /*
  int d[] = {0,1,1,0,0,0,0,0}; //domingo - segunda
  int h[] = {1};
  int m[] = {30}; 
  */
  int d = 6;
  ...
  //for(i=1; i<=7; i++){
      if(d == diasemana && h == hora && (minuto-2 < m && m > minuto +2)){  //definir intervalo de tempo para garantir que ocorrera o aviso
        int i = 0;
        while(digitalRead(BUT)== LOW){  //repetir 5 vezes  - tirar botao 
          
          lcd.setCursor(0,0);
          lcd.print(nome);
          lcd.setCursor(0,1);      // esperar 5 segundos!!!
          lcd.print(qtdd);
